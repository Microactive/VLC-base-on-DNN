...
def save(if_train_new):
    ...
    if if_train_new:
        ...
        TrainLabel = torch.from_numpy(np.loadtxt('TrainLabel_fromrealworld.csv', delimiter=',', dtype=np.float32))
        TrainData = torch.from_numpy(np.loadtxt('TrainData_fromrealworld.csv', delimiter=',', dtype=np.float32))
        TestLabel = np.loadtxt('TestLabel_fromrealworld.csv', delimiter=',', dtype=np.float32)
        TestData = torch.from_numpy(np.loadtxt('TestData_fromrealworld.csv', delimiter=',', dtype=np.float32))
        trainlabel = Variable(TrainLabel)
        traindata = Variable(TrainData)
        testdata = Variable(TestData)
